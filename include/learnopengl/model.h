...
class Model {
	    ...
	    void processNode(aiNode *node, const aiScene *scene) {
			...
			for ( unsigned int i = 0; i < node->mNumMeshes; i++ ) {
				    // the node object only contains indices to
				    // index the actual objects in the scene.
				    // the scene contains all the data, node is
				    // just to keep stuff organized (like
				    // relations between nodes).
				    aiMesh *mesh =
					scene->mMeshes[node->mMeshes[i]];
				    meshes.push_back(processMesh(mesh, scene));
			}
			// after we've processed all of the meshes (if any) we
	    ...
	    Mesh processMesh(aiMesh *mesh, const aiScene *scene) {
			...
			for ( unsigned int i = 0; i < mesh->mNumVertices;
			      i++ ) {
				    Vertex vertex;
				    glm::vec3
					vector; // we declare a placeholder
						// vector since assimp_ uses its
						// own vector class that doesn't
						// directly convert to glm's
						// vec3 class so we transfer the
						// data to this placeholder
						// glm::vec3 first.
				    // positions
				    ...
				    if ( mesh->mTextureCoords
					     [0] ) // does the mesh contain
						   // texture coordinates?
				    {
						glm::vec2 vec;
						// a vertex can contain up to 8
						// different texture
						// coordinates. We thus make the
						// assumption that we won't use
						// models where a vertex can
						// have multiple texture
						// coordinates so we always take
						// the first set (0).
						vec.x =
			...
			// now wak through each of the mesh's faces (a face is a
			// mesh its triangle) and retrieve the corresponding
			// vertex indices.
			...
			// we assume a convention for sampler names in the
			// shaders. Each diffuse texture should be named as
			// 'texture_diffuseN' where N is a sequential number
			// ranging from 1 to MAX_SAMPLER_NUMBER. Same applies to
			...
			// return a mesh object created from the extracted mesh
			// data
	    ...
	    // textures if they're not loaded yet. the required info is returned
	    // as a Texture struct.
	    vector<Texture> loadMaterialTextures(aiMaterial *mat,
			...
			for ( unsigned int i = 0;
			      i < mat->GetTextureCount(type); i++ ) {
				    aiString str;
				    mat->GetTexture(type, i, &str);
				    // check if texture was loaded before and if
				    // so, continue to next iteration: skip
				    // loading a new texture
				    bool skip = false;
				    for ( unsigned int j = 0;
					  j < textures_loaded.size(); j++ ) {
						if ( std::strcmp(
							 textures_loaded[j]
							     .path.data(),
							 str.C_Str()) == 0 ) {
				    ...
				    if ( !skip ) { // if texture hasn't been
						   // loaded already, load it
						Texture texture;
