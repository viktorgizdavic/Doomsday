...
class ModelObject : public Model {
	    ...
	    public:
	    ModelObject(string const &path, bool gamma = false)
		: Model(path, gamma) {}

	    void setup(Shader &shader, glm::mat4 projection, glm::mat4 view,
		       glm::vec3 viewPos, DirLight &dirLight,
			...
			for ( int i = 0; i < pointLights.size(); ++i ) {
				    ...
				    shader.setVec3("pointLights[" + iStr +
						       "].position",
				    ...
				    shader.setFloat("pointLights[" + iStr +
							"].constant",
			...
			shader.setVec3("spotLight.position",
				       spotLight.position);
			shader.setVec3("spotLight.direction",
				       spotLight.direction);
			...
			shader.setVec3("spotLight.specular",
				       spotLight.specular);
			shader.setFloat("spotLight.constant",
					spotLight.constant);
			shader.setFloat("spotLight.linear", spotLight.linear);
			shader.setFloat("spotLight.quadratic",
					spotLight.quadratic);
	    ...
	    void setRotateVect(float angle, glm::vec3 vector) {
			rotationVect = glm::rotate(rotationVect,
						   glm::radians(angle), vector);
	    }

	    void rotate() { position *= rotationVect; }

	    void scale(glm::vec3 vector) {
			position = glm::scale(position, vector);
	    }

	    void scale() { position = glm::scale(position, scaleVect); }

	    void setScaleVect(glm::vec3 vector) { scaleVect = vector; }

	    void setTranslateVect(glm::vec3 vector) { translateVect = vector; }

	    glm::vec3 getTranslateVect() { return translateVect; }
