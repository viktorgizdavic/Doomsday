...
#define IM_ASSERT(_EXPR)                                                       \
	    assert(_EXPR) // You can override the default assert handler by
			  // editing imconfig.h
#endif
...
#define IM_OFFSETOF(_TYPE, _MEMBER)                                            \
	    offsetof(_TYPE, _MEMBER) // Offset of _MEMBER within _TYPE.
				     // Standardized as offsetof() in C++11
#else
#define IM_OFFSETOF(_TYPE, _MEMBER)                                            \
	    ((size_t) & (((_TYPE *)0)->_MEMBER)) // Offset of _MEMBER within
						 // _TYPE. Old style macro.
#endif
#if !defined(IMGUI_USE_STB_SPRINTF) && defined(__clang__)
#define IM_FMTARGS(FMT)                                                        \
	    __attribute__((                                                    \
		format(printf, FMT, FMT + 1))) // Apply printf-style warnings to
					       // our formatting functions.
#define IM_FMTLIST(FMT) __attribute__((format(printf, FMT, 0)))
#elif !defined(IMGUI_USE_STB_SPRINTF) && defined(__GNUC__) &&                  \
    defined(__MINGW32__)
#define IM_FMTARGS(FMT)                                                        \
	    __attribute__((format(gnu_printf, FMT,                             \
				  FMT + 1))) // Apply printf-style warnings to
					     // our formatting functions.
...
struct ImVec2 {
	    ...
	    float operator[](size_t idx) const {
			IM_ASSERT(idx <= 1);
			return (&x)[idx];
	    } // We very rarely use this [] operator, the assert overhead is
	      // fine.
...
enum ImGuiWindowFlags_ {
		...
		1 << 3, // Disable scrollbars (window can still scroll with
			// mouse or programmatically)
	    ImGuiWindowFlags_NoScrollWithMouse =
		1 << 4, // Disable user vertically scrolling with mouse wheel.
			// On child window, mouse wheel will be forwarded to the
		...
		1 << 9, // Disable catching mouse, hovering test with pass
			// through.
		...
		1 << 14, // Always show vertical scrollbar (even if
		...
		1 << 20, // Append '*' to title without affecting the ID, as a
			 // convenience to avoid using the ### operator. When
			 // used in a tab/docking context, tab is selected on
			 // closure and closure is deferred by one frame to
			 // allow code to cancel the closure (with a
		...
		1 << 23, // [BETA] Allow gamepad/keyboard navigation to cross
			 // over parent border to this child (only use on child
			 // that have no scrolling!)
	    ...
	    // io.ConfigWindowsResizeFromEdges=true and make sure mouse cursors
	    // are supported by backend (io.BackendFlags &
...
enum ImGuiInputTextFlags_ {
	    ...
	    ImGuiInputTextFlags_CharsNoBlank = 1
					    << 3, // Filter out spaces, tabs
	    ImGuiInputTextFlags_AutoSelectAll =
		1 << 4, // Select entire text when first taking mouse focus
	    ImGuiInputTextFlags_EnterReturnsTrue =
		1 << 5, // Return 'true' when Enter is pressed (as opposed to
			// every time the value was modified). Consider looking
			// at the IsItemDeactivatedAfterEdit() function.
	    ImGuiInputTextFlags_CallbackCompletion =
		1 << 6, // Callback on pressing TAB (for completion handling)
	    ImGuiInputTextFlags_CallbackHistory =
		1 << 7, // Callback on pressing Up/Down arrows (for history
			// handling)
	    ImGuiInputTextFlags_CallbackAlways =
		1 << 8, // Callback on each iteration. User code may query
			// cursor position, modify text buffer.
	    ImGuiInputTextFlags_CallbackCharFilter =
		1 << 9, // Callback on character inputs to replace or discard
			// them. Modify 'EventChar' to replace or discard, or
			// return 1 in callback to discard.
	    ImGuiInputTextFlags_AllowTabInput =
		1 << 10, // Pressing TAB input a '\t' character into the text
			 // field
		...
		1 << 18, // Callback on buffer capacity changes request (beyond
			 ...
			 // grow. Notify when the string wants to be resized
			 // (for string types which hold a cache of their Size).
			 // You will be provided a new BufSize in the callback
			 // and NEED to honor it. (see misc/cpp/imgui_stdlib.h
			 // for an example of using this)
	    ImGuiInputTextFlags_CallbackEdit =
		1 << 19, // Callback on any edit (note that InputText() already
			 // returns true on edit, the callback is useful mainly
			 // to manipulate the underlying buffer while focus is
			 // active)
	    // [Internal]
	    ImGuiInputTextFlags_Multiline =
		1 << 20, // For internal use by InputTextMultiline()
	    ImGuiInputTextFlags_NoMarkEdited =
		1 << 21 // For internal use by functions using InputText()
			// before reformatting data
};
...
enum ImGuiTreeNodeFlags_ {
		...
		1
		<< 1, // Draw frame with background (e.g. for CollapsingHeader)
	    ImGuiTreeNodeFlags_AllowItemOverlap =
		1 << 2, // Hit testing to allow subsequent widgets to overlap
			// this one
	    ...
	    ImGuiTreeNodeFlags_Leaf = 1 << 8, // No collapsing, no arrow (use as
					      // a convenience for leaf nodes).
	    ImGuiTreeNodeFlags_Bullet = 1
				     << 9, // Display a bullet instead of arrow
		...
		1 << 11, // Extend hit box to the right-most edge, even if not
			 ...
			 // adding other items on the same line. In the future
			 // we may refactor the hit system to be front-to-back,
			 // allowing natural overlaps and then this can become
			 // the default.
	    ImGuiTreeNodeFlags_SpanFullWidth =
		1 << 12, // Extend hit box to the left-most and right-most edges
			 // (bypass the indented area).
	    ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
		1 << 13, // (WIP) Nav: left direction may move to this
...
enum ImGuiPopupFlags_ {
		...
		1 << 5, // For OpenPopup*(), BeginPopupContext*(): don't open if
			// there's already a popup at the same level of the
			// popup stack
	    ImGuiPopupFlags_NoOpenOverItems =
		1 << 6, // For BeginPopupContextWindow(): don't return true when
			// hovering items, only when hovering empty space
	    ImGuiPopupFlags_AnyPopupId =
		1 << 7, // For IsPopupOpen(): ignore the ImGuiID parameter and
			// test for any popup.
	    ImGuiPopupFlags_AnyPopupLevel =
		1 << 8, // For IsPopupOpen(): search/test at any level of the
...
enum ImGuiSelectableFlags_ {
		...
		1 << 1, // Selectable frame can span all columns (text will
		...
		1 << 4 // (WIP) Hit testing to allow subsequent widgets to
		       // overlap this one
};
...
enum ImGuiTabBarFlags_ {
		...
		<< 3, // Disable behavior of closing tabs (that are submitted
		      // with p_open != NULL) with middle mouse button. You can
		      // still repro this behavior on user's side with if
...
enum ImGuiTabItemFlags_ {
		...
		1 << 0, // Append '*' to title without affecting the ID, as a
			// convenience to avoid using the ### operator. Also:
			// tab is selected on closure and closure is deferred by
			// one frame to allow code to undo it without flicker.
	    ImGuiTabItemFlags_SetSelected =
		1 << 1, // Trigger flag to programmatically make the tab
		...
		1 << 3, // Don't call PushID(tab->ID)/PopID() on
...
enum ImGuiTableFlags_ {
		...
		1 << 1, // Allow reordering columns in header row (need calling
			// TableSetupColumn() + TableHeadersRow() to display
			// headers)
	    ImGuiTableFlags_Hideable =
		1 << 2, // Allow hiding/disabling columns in context menu.
	    ImGuiTableFlags_Sortable =
		1 << 3, // Allow sorting on one column (sort_specs_count will
			// always be == 1). Call TableGetSortSpecs() to obtain
			// sort specs.
	    ImGuiTableFlags_MultiSortable =
		1 << 4, // Allow sorting on multiple columns by holding Shift
			// (sort_specs_count may be > 1). Call
			// TableGetSortSpecs() to obtain sort specs.
	    ImGuiTableFlags_NoSavedSettings =
		1 << 5, // Disable persisting columns order, width and sort
			// settings in the .ini file.
	    ImGuiTableFlags_ContextMenuInBody =
		1 << 6, // Right-click on columns body/contents will display
		...
		1 << 12, // Disable vertical borders in columns Body (borders
			 // will always appears in Headers).
	    ImGuiTableFlags_NoBordersInBodyUntilResize =
		1 << 13, // Disable vertical borders in columns Body until
			 // hovered for resize (borders will always appears in
			 // Headers). Sizing
	    ImGuiTableFlags_ColumnsWidthStretch =
		1 << 14, // Default if ScrollX is off. Columns will default to
			 // use _WidthStretch. Read description above for more
			 // details.
	    ImGuiTableFlags_ColumnsWidthFixed =
		1
		<< 15, // Default if ScrollX is on. Columns will default to use
		       // _WidthFixed or _WidthAutoResize policy (if Resizable
		       // is off). Read description above for more details.
	    ImGuiTableFlags_SameWidths =
		1 << 16, // Make all columns the same widths which is useful
			 // with Fixed columns policy (but granted by default
			 // with Stretch policy + no resize). Implicitly enable
		...
		1 << 18, // Disable extending past the limit set by
			 // outer_size.y, only meaningful when neither of
			 // ScrollX|ScrollY are set (data below the limit will
		...
		1 << 20, // Disable distributing remainder width to stretched
			 ...
			 // columns: Without this flag: 33,33,34. With this
			 // flag: 33,33,33). With larger number of columns,
			 // resizing will appear to be less smooth.
	    ImGuiTableFlags_NoClip =
		1 << 21, // Disable clipping rectangle for every individual
	    ...
	    ImGuiTableFlags_PadOuterX = 1
				     << 22, // Default if BordersOuterV is on.
					    // Enable outer-most padding.
	    ImGuiTableFlags_NoPadOuterX =
		1 << 23, // Default if BordersOuterV is off. Disable outer-most
			 // padding.
		...
		1 << 25, // Enable horizontal scrolling. Require 'outer_size'
			 ...
			 // create a child window, ScrollY is currently
			 // generally recommended when using ScrollX.
	    ImGuiTableFlags_ScrollY =
		1 << 26 // Enable vertical scrolling. Require 'outer_size'
			// parameter of BeginTable() to specify the container
			// size.
};
...
enum ImGuiTableColumnFlags_ {
		...
		1 << 12, // Header width don't contribute to automatic column
			 // width.
	    ImGuiTableColumnFlags_PreferSortAscending =
		1 << 13, // Make the initial sort direction Ascending when first
			 // sorting on this column (default).
	    ImGuiTableColumnFlags_PreferSortDescending =
		1 << 14, // Make the initial sort direction Descending when
			 // first sorting on this column.
	    ImGuiTableColumnFlags_IndentEnable =
		1 << 15, // Use current Indent value when entering cell (default
			 // for column 0).
	    ImGuiTableColumnFlags_IndentDisable =
		1 << 16, // Ignore current Indent value when entering cell
		...
		1
		<< 20, // Status: is enabled == not hidden by user/api (referred
...
enum ImGuiTableRowFlags_ {
		...
		1 << 0 // Identify header row (set default background color +
		       // width of its contents accounted different for auto
		       // column width)
};
...
enum ImGuiTableBgTarget_ {
	    ...
	    // FIXME-TABLE: Todo. Set column background color 0 (generally used
	    // for background ImGuiTableBgTarget_ColumnBg1                  = 2,
	    // // FIXME-TABLE: Todo. Set column background color 1 (generally
		...
		4, // Set row background color 1 (generally used for selection
		   // marking)
...
enum ImGuiFocusedFlags_ {
		...
		1 << 1, // IsWindowFocused(): Test from root window (top most
			// parent of the current hierarchy)
	    ImGuiFocusedFlags_AnyWindow =
		1 << 2, // IsWindowFocused(): Return true if any window is
			// focused. Important: If you are trying to tell how to
			// dispatch your low-level inputs, do NOT use this. Use
...
enum ImGuiHoveredFlags_ {
		...
		1 << 3, // Return true even if a popup window is normally
			// blocking access to this item/window
	    // ImGuiHoveredFlags_AllowWhenBlockedByModal     = 1 << 4,   //
	    // Return true even if a modal popup window is normally blocking
	    // access to this item/window. FIXME-TODO: Unavailable yet.
	    ImGuiHoveredFlags_AllowWhenBlockedByActiveItem =
		1
		<< 5, // Return true even if an active item is blocking access
...
enum ImGuiDragDropFlags_ {
		...
		1 << 0, // By default, a successful call to BeginDragDropSource
			// opens a tooltip so you can display a preview or
			// description of the source contents. This flag disable
			// this behavior.
	    ImGuiDragDropFlags_SourceNoDisableHover =
		1 << 1, // By default, when dragging we clear data so that
			// IsItemHovered() will return false, to avoid
		...
		<< 3, // Allow items such as Text(), Image() that have no unique
		      // identifier to be used as drag source, by manufacturing
		      // a temporary identifier based on their window-relative
		...
		1 << 10, // AcceptDragDropPayload() will returns true even
			 // before the mouse button is released. You can then
			 // call IsDelivery() to test if the payload needs to be
			 // delivered.
...
enum ImGuiNavInput_ {
	    ...
	    ImGuiNavInput_Activate, // activate / open / toggle / tweak value //
				    ...
				    // Space (Keyboard)
	    ImGuiNavInput_Cancel, // cancel / close / exit // e.g. Circle (PS4),
				  // B (Xbox), B (Switch), Escape (Keyboard)
	    ...
	    ImGuiNavInput_Menu, // tap: toggle menu / hold: focus, move, resize
				// // e.g. Square (PS4), X (Xbox), Y (Switch),
				// Alt (Keyboard)
	    ImGuiNavInput_DpadLeft, // move / tweak / resize window (w/ PadMenu)
				    // // e.g. D-pad Left/Right/Up/Down
				    // (Gamepads), Arrow keys (Keyboard)
	    ImGuiNavInput_DpadRight,  //
	    ImGuiNavInput_DpadUp,     //
	    ImGuiNavInput_DpadDown,   //
	    ImGuiNavInput_LStickLeft, // scroll / move window (w/ PadMenu) //
				      // e.g. Left Analog Stick
				      // Left/Right/Up/Down
	    ImGuiNavInput_LStickRight, //
	    ImGuiNavInput_LStickUp,    //
	    ImGuiNavInput_LStickDown,  //
	    ImGuiNavInput_FocusPrev,   // next window (w/ PadMenu) // e.g. L1 or
				     // L2 (PS4), LB or LT (Xbox), L or ZL
				     // (Switch)
	    ImGuiNavInput_FocusNext, // prev window (w/ PadMenu) // e.g. R1 or
				     // R2 (PS4), RB or RT (Xbox), R or ZL
				     // (Switch)
	    ImGuiNavInput_TweakSlow, // slower tweaks // e.g. L1 or L2 (PS4), LB
				     // or LT (Xbox), L or ZL (Switch)
	    ImGuiNavInput_TweakFast, // faster tweaks // e.g. R1 or R2 (PS4), RB
				     // or RT (Xbox), R or ZL (Switch)

	    // [Internal] Don't use directly! This is used internally to
	    // differentiate keyboard from gamepad inputs for behaviors that
	    // require to differentiate them. Keyboard behavior that have no
	    // corresponding gamepad mapping (e.g. CTRL+TAB) will be directly
	    // reading from io.KeysDown[] instead of io.NavInputs[].
...
enum ImGuiConfigFlags_ {
		...
		1 << 0, // Master keyboard navigation enable flag. NewFrame()
			// will automatically fill io.NavInputs[] based on
			// io.KeysDown[].
	    ImGuiConfigFlags_NavEnableGamepad =
		1
		<< 1, // Master gamepad navigation enable flag. This is mostly
		      // to instruct your imgui backend to fill io.NavInputs[].
		      // Backend also needs to set ImGuiBackendFlags_HasGamepad.
	    ImGuiConfigFlags_NavEnableSetMousePos =
		1 << 2, // Instruct navigation to move the mouse cursor. May be
			// useful on TV/console systems where moving a virtual
			// mouse is awkward. Will update io.MousePos and set
			// io.WantSetMousePos=true. If enabled you MUST honor
			// io.WantSetMousePos requests in your backend,
			// otherwise ImGui will react as if the mouse is jumping
		...
		1 << 4, // Instruct imgui to clear mouse position/buttons in
			// NewFrame(). This allows ignoring the mouse
			// information set by the backend.
...
enum ImGuiColorEditFlags_ {
		...
		1
		<< 5, //              // ColorEdit, ColorPicker: disable inputs
		...
		1 << 9, //              // ColorEdit: disable drag and drop
			//              target. ColorButton: disable drag and
			//              drop source.
	    ImGuiColorEditFlags_NoBorder =
		1 << 10, //              // ColorButton: disable border (which
			 //              is enforced by default)

	    // User Options (right-click on widget to change some of them).
	    ImGuiColorEditFlags_AlphaBar =
		1 << 16, //              // ColorEdit, ColorPicker: show
		...
		1 << 18, //              // ColorEdit, ColorPicker, ColorButton:
			 //              display half opaque / half
			 //              checkerboard, instead of opaque.
	    ImGuiColorEditFlags_HDR =
		1
		<< 19, //              // (WIP) ColorEdit: Currently only
		...
		1 << 27, // [Input]      // ColorEdit, ColorPicker: input and
	    ...
	    // SetColorEditOptions(). The intent is that you probably don't want
	    // to override them in most of your calls. Let the user choose via
	    // the option menu and/or call SetColorEditOptions() once during
	    // startup.
...
enum ImGuiSliderFlags_ {
		...
		1 << 4, // Clamp value to min/max bounds when input manually
			// with CTRL+Click. By default CTRL+Click allows going
			// out of bounds.
	    ImGuiSliderFlags_Logarithmic =
		1 << 5, // Make the widget logarithmic (linear otherwise).
			// Consider using ImGuiSliderFlags_NoRoundToFormat with
			// this if using a format-string with small amount of
			// digits.
		...
		1 << 7, // Disable CTRL+Click or Enter key allowing to input
			// text directly into the widget
	    ImGuiSliderFlags_InvalidMask_ =
		0x7000000F // [Internal] We treat using those bits as being
			   // potentially a 'float power' argument from the
			   // previous API that has got miscast to this enum,
...
enum ImGuiMouseCursor_ {
	    ...
	    ImGuiMouseCursor_ResizeNS,	 // When hovering over an horizontal
					 // border
...
enum ImGuiCond_ {
	    ...
	    ImGuiCond_Once = 1
			  << 1, // Set the variable once per runtime session
...
template <typename T> struct ImVector {
	    ...
	    inline void reserve(int new_capacity) {
			if ( new_capacity <= Capacity )
				    return;
			T *new_data =
			    (T *)IM_ALLOC((size_t)new_capacity * sizeof(T));
			if ( Data ) {
	    ...
	    inline T *erase(const T *it, const T *it_last) {
			IM_ASSERT(it >= Data && it < Data + Size &&
				  it_last > it && it_last <= Data + Size);
			const ptrdiff_t count = it_last - it;
			const ptrdiff_t off = it - Data;
			memmove(Data + off, Data + off + count,
				((size_t)Size - (size_t)off - count) *
				    sizeof(T));
			Size -= (int)count;
			return Data + off;
	    }
...
struct ImGuiStyle {
	    ...
	    float WindowBorderSize; // Thickness of border around windows.
				    ...
				    // CPU/GPU costly).
	    ImVec2
		WindowMinSize; // Minimum window size. This is a global setting.
			       // If you want to constraint individual windows,
			       // use SetNextWindowSizeConstraints().
	    ImVec2 WindowTitleAlign; // Alignment for title bar text. Defaults
				     // to (0.0f,0.5f) for
	    ...
	    ImVec2 FramePadding;   // Padding within a framed rectangle (used by
				   // most widgets).
	    float FrameRounding;   // Radius of frame corners rounding. Set to
				 // 0.0f to have rectangular frame (used by most
				 // widgets).
	    float
		...
		TouchExtraPadding; // Expand reactive bounding box for
				   ...
				   // not accurate enough. Unfortunately we
				   // don't sort widgets so priority on overlap
				   // will always be given to the first widget.
	    ...
	    float ColumnsMinSpacing; // Minimum horizontal spacing between two
				     // columns. Preferably > (FramePadding.x +
				     // 1).
	    ...
	    float TabMinWidthForCloseButton; // Minimum width for close button
					     // to appears on an unselected tab
					     // when hovered. Set to 0.0f to
					     // always show when hovering, set
					     // to FLT_MAX to never show close
	    ...
	    ImVec2 ButtonTextAlign; // Alignment of button text when button is
				    // larger than text. Defaults to (0.5f,
				    // 0.5f) (centered).
	    ImVec2 SelectableTextAlign; // Alignment of selectable text.
					// Defaults to (0.0f, 0.0f) (top-left
					// aligned). It's generally important to
					// keep this left-aligned if you want to
					// lay multiple items on a same line.
	    ImVec2 DisplayWindowPadding; // Window position are clamped to be
					 // visible within the display area or
					 // monitors by at least this amount.
					 // Only applies to regular windows.
	    ImVec2
		DisplaySafeAreaPadding; // If you cannot see the edges of your
					// screen (e.g. on a TV) increase the
					// safe area padding. Apply to
					// popups/tooltips as well regular
					// windows. NB: Prefer configuring your
					// TV sets correctly!
	    float MouseCursorScale;	// Scale software rendered mouse cursor
				    // (when io.MouseDrawCursor is enabled). May
				    // be removed later.
	    ...
	    bool AntiAliasedLinesUseTex; // Enable anti-aliased lines/borders
					 ...
					 // bilinear filtering. Latched at the
	    ...
	    float CurveTessellationTol; // Tessellation tolerance when using
					// PathBezierCurveTo() without a
					// specific number of segments. Decrease
					// for highly tessellated curves (higher
					// quality, more polygons), increase to
					// reduce quality.
	    float
...
struct ImGuiIO {
		...
		ConfigFlags; // = 0              // See ImGuiConfigFlags_ enum.
			     // Set by user/application. Gamepad/keyboard
			     // navigation options, etc.
	    ImGuiBackendFlags
		BackendFlags;	// = 0              // See ImGuiBackendFlags_
				// enum. Set by backend (imgui_impl_xxx files or
				// custom backend) to communicate features
				// supported by the backend.
	    ImVec2 DisplaySize; // <unset>          // Main display size, in
				// pixels.
	    float DeltaTime;	// = 1.0f/60.0f     // Time elapsed since last
				// frame, in seconds.
	    float IniSavingRate; // = 5.0f           // Minimum time between
				 // saving positions/sizes to .ini file, in
				 // seconds.
	    const char *
		IniFilename; // = "imgui.ini"    // Path to .ini file. Set NULL
	    ...
	    int KeyMap[ImGuiKey_COUNT]; // <unset>          // Map of indices
					// into the KeysDown[512] entries array
					// which represent your "native"
					// keyboard state.
	    float KeyRepeatDelay;	// = 0.250f         // When holding a
					// key/button, time before it starts
					// repeating, in seconds (for buttons in
					// Repeat mode, etc.).
	    float
		...
		*Fonts; // <auto>           // Font atlas: load, rasterize and
	    ...
	    bool FontAllowUserScaling; // = false          // Allow user scaling
				       ...
				       // CTRL+Wheel.
	    ImFont
		*FontDefault; // = NULL           // Font to use on NewFrame().
			      // Use NULL to uses Fonts->Fonts[0].
	    ImVec2 DisplayFramebufferScale; // = (1, 1)         // For retina
					    // display or other situations where
					    // window coordinates are different
					    // from framebuffer coordinates.
	    ...
	    bool MouseDrawCursor; // = false          // Request ImGui to draw a
				  ...
				  // implementations.
	    bool
		ConfigMacOSXBehaviors; // = defined(__APPLE__) // OS X style:
				       // Text editing cursor movement using Alt
				       // instead of Ctrl, Shortcuts using
				       // Cmd/Super instead of Ctrl, Line/Text
				       // Start and End using Cmd+Arrows instead
				       // of Home/End, Double click selects by
				       // word instead of selecting whole text,
				       // Multi-selection in lists uses
				       // Cmd/Super instead of Ctrl (was called
				       // io.OptMacOSXBehaviors prior to 1.63)
	    bool ConfigInputTextCursorBlink; // = true           // Set to false
					     // to disable blinking cursor, for
					     // users who consider it
					     // distracting. (was called:
	    ...
	    bool ConfigWindowsResizeFromEdges; // = true           // Enable
					       ...
					       // flag)
	    bool ConfigWindowsMoveFromTitleBarOnly; // = false       // [BETA]
						    // Set to true to only allow
						    // moving windows when
						    // clicked+dragged from the
						    // title bar. Windows
						    // without a title bar are
						    // not affected.
	    float ConfigMemoryCompactTimer; // = 60.0f          // [BETA] Free
					    // transient windows/tables memory
					    // buffers when unused for given
					    // amount of time. Set to -1.0f to
					    // disable.
	    ...
	    // Optional: Platform/Renderer backend name (informational only!
	    // will be displayed in About Window) + User data for
	    ...
	    // your cursor for text input position (e.g. when using
	    // Japanese/Chinese IME on Windows) (default to use native imm32 api
	    // on Windows)
	    void (*ImeSetInputScreenPosFn)(int x, int y);
	    void *ImeWindowHandle; // = NULL           // (Windows) Set this to
				   // your HWND to get automatic IME cursor
				   // positioning.
	    ...
	    bool MouseDown[5]; // Mouse buttons: 0=left, 1=right, 2=middle +
			       ...
			       // ImGui mostly uses left and right buttons.
			       // Others buttons allows us to track if the mouse
			       // is being used by your application + available
			       // to user as a convenience via IsMouse** API.
	    float MouseWheel;  // Mouse wheel Vertical: 1 unit scrolls about 5
			       // lines text.
	    float MouseWheelH; // Mouse wheel Horizontal. Most users don't have
			       // a mouse with an horizontal wheel, may not be
			       // filled by all backends.
	    bool KeyCtrl;      // Keyboard modifier pressed: Control
	    bool KeyShift;     // Keyboard modifier pressed: Shift
	    bool KeyAlt;       // Keyboard modifier pressed: Alt
	    bool KeySuper;     // Keyboard modifier pressed: Cmd/Super/Windows
	    bool KeysDown[512]; // Keyboard keys that are pressed (ideally left
				// in the "native" order your engine has access
				// to keyboard keys, so you can use your own
				// defines/enums for keys).
	    float NavInputs[ImGuiNavInput_COUNT]; // Gamepad inputs. Cleared
						  // back to zero by EndFrame().
						  // Keyboard keys will be
						  // auto-mapped and be written
						  // here by NewFrame().
		...
		ImWchar16 c); // Queue new character input from an UTF-16
			      // character, it can be a surrogate
	    IMGUI_API void AddInputCharactersUTF8(
		const char
		    *str); // Queue new characters input from an UTF-8 string
	    IMGUI_API void
	    ClearInputCharacters(); // Clear the text input buffer manually

	    //------------------------------------------------------------------
	    // Output - Updated by NewFrame() or EndFrame()/Render()
	    // (when reading from the io.WantCaptureMouse,
	    // io.WantCaptureKeyboard flags to dispatch your inputs, it is
	    //  generally easier and more correct to use their state BEFORE
	    //  calling NewFrame(). See FAQ for details!)
	    //------------------------------------------------------------------
	    ...
	    bool WantCaptureMouse; // Set when Dear ImGui will use mouse inputs,
				   ...
				   // window, widget is active, mouse was
	    ...
	    bool WantCaptureKeyboard; // Set when Dear ImGui will use keyboard
				      ...
				      // (either way, always pass keyboard
				      // inputs to imgui). (e.g. InputText
				      // active, or an imgui window is focused
				      // and navigation is enabled, etc.).
	    bool
		WantTextInput; // Mobile/console: when set, you may display an
	    ...
	    bool WantSetMousePos; // MousePos has been altered, backend should
				  // reposition mouse on next frame. Rarely
				  // used! Set only when
				  // ImGuiConfigFlags_NavEnableSetMousePos flag
				  // is enabled.
	    ...
	    bool NavActive;  // Keyboard/Gamepad navigation is currently allowed
			     // (will handle ImGuiKey_NavXXX events) = a window
			     // is focused and it doesn't use the
			     // ImGuiWindowFlags_NoNavInputs flag.
	    bool NavVisible; // Keyboard/Gamepad navigation is visible and
			     // allowed (will handle ImGuiKey_NavXXX events).
	    float
		Framerate; // Application framerate estimate, in frame per
	    ...
	    int MetricsActiveAllocations; // Number of active allocations,
					  // updated by MemAlloc/MemFree based
					  // on current context. May be off if
					  // you have multiple imgui contexts.
	    ImVec2 MouseDelta; // Mouse delta. Note that this is zero if either
			       // current or previous position are invalid
			       // (-FLT_MAX,-FLT_MAX), so a
			       // disappearing/reappearing mouse won't have a
			       // huge delta.
	    ...
	    ImGuiKeyModFlags
		KeyMods; // Key mods flags (same as
			 // io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into
			 // flags), updated by NewFrame()
	    ImVec2
		MousePosPrev; // Previous mouse position (note that MouseDelta
			      // is not necessary == MousePos-MousePosPrev, in
			      // case either position is invalid)
	    ImVec2 MouseClickedPos[5];	// Position at time of clicking
	    double MouseClickedTime[5]; // Time of last click (used to figure
					// out double-click)
	    bool MouseClicked[5];	// Mouse button went from !Down to Down
	    bool MouseDoubleClicked[5]; // Has mouse button been double-clicked?
	    bool MouseReleased[5];	// Mouse button went from Down to !Down
	    bool MouseDownOwned[5]; // Track if button was clicked inside a dear
				    // imgui window. We don't request mouse
				    // capture from the application if click
	    ...
	    float MouseDownDurationPrev[5]; // Previous time the mouse button
					    // has been down
	    ImVec2
	    ...
	    float MouseDragMaxDistanceSqr[5]; // Squared maximum distance of how
					      // much mouse has traveled from
					      // the clicking point
	    float KeysDownDuration[512]; // Duration the keyboard key has been
					 // down (0.0f == just pressed)
	    float KeysDownDurationPrev[512]; // Previous duration the key has
					     // been down
	    ...
	    ImVector<ImWchar>
		InputQueueCharacters; // Queue of _characters_ input (obtained
				      // by platform backend). Fill using
...
struct ImGuiInputTextCallbackData {
	    ...
	    // buffer allocation, you may modify the contents of 'Buf[]'
	    // directly. You need to update 'BufTextLen' accordingly (0 <=
	    // BufTextLen < BufSize) and set 'BufDirty'' to true so InputText
	    ...
	    char *Buf; // Text buffer                          // Read-write //
		       ...
		       // [Completion,History,Always] Only write to pointed
		       // data, don't replace the actual pointer!
	    int BufTextLen; // Text length (in bytes)               //
			    // Read-write   //
...
struct ImGuiTableColumnSortSpecs {
	    ...
	    ImS16 SortOrder; // Index within parent ImGuiTableSortSpecs (always
			     // stored in order starting from 0, tables sorted
		...
		SortDirection : 8; // ImGuiSortDirection_Ascending or
				   ...
				   // this or SortSign, whichever is more
...
struct ImGuiTableSortSpecs {
		...
		*Specs;	     // Pointer to sort spec array.
	    ...
	    bool SpecsDirty; // Set to true when specs have changed since last
			     // time! Use this to sort again, then clear the
			     // flag.
...
static inline bool DragFloat2(const char *label, float v[2], float v_speed,
			      float v_min, float v_max, const char *format,
			      float power) {
	    return DragScalarN(label, ImGuiDataType_Float, v, 2, v_speed,
			       &v_min, &v_max, format, power);
}
static inline bool DragFloat3(const char *label, float v[3], float v_speed,
			      float v_min, float v_max, const char *format,
			      float power) {
	    return DragScalarN(label, ImGuiDataType_Float, v, 3, v_speed,
			       &v_min, &v_max, format, power);
}
static inline bool DragFloat4(const char *label, float v[4], float v_speed,
			      float v_min, float v_max, const char *format,
			      float power) {
	    return DragScalarN(label, ImGuiDataType_Float, v, 4, v_speed,
			       &v_min, &v_max, format, power);
}
IMGUI_API bool SliderScalar(const char *label, ImGuiDataType data_type,
			    void *p_data, const void *p_min, const void *p_max,
...
static inline bool SliderFloat2(const char *label, float v[2], float v_min,
				float v_max, const char *format, float power) {
	    return SliderScalarN(label, ImGuiDataType_Float, v, 2, &v_min,
				 &v_max, format, power);
}
static inline bool SliderFloat3(const char *label, float v[3], float v_min,
				float v_max, const char *format, float power) {
	    return SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min,
				 &v_max, format, power);
}
static inline bool SliderFloat4(const char *label, float v[4], float v_min,
				float v_max, const char *format, float power) {
	    return SliderScalarN(label, ImGuiDataType_Float, v, 4, &v_min,
				 &v_max, format, power);
}
...
static inline bool BeginPopupContextWindow(const char *str_id,
		...
		str_id,
		mb | (over_items ? 0 : ImGuiPopupFlags_NoOpenOverItems));
}
...
struct ImGuiTextBuffer {
	    ...
	    IMGUI_API void appendfv(const char *fmt, va_list args)
		IM_FMTLIST(2);
};
...
struct ImGuiStorage {
	    ...
	    // - Get***() functions find pair, never add/allocate. Pairs are
	    // sorted so a query is O(log N)
	    // - Set***() functions find pair, insertion on demand if missing.
	    // - Sorted insertion is costly, paid once. A typical frame
	    // shouldn't need to insert any new pair.
	    void Clear() { Data.clear(); }
	    IMGUI_API int GetInt(ImGuiID key, int default_val = 0) const;
	    IMGUI_API void SetInt(ImGuiID key, int val);
	    IMGUI_API bool GetBool(ImGuiID key, bool default_val = false) const;
	    IMGUI_API void SetBool(ImGuiID key, bool val);
	    IMGUI_API float GetFloat(ImGuiID key,
				     float default_val = 0.0f) const;
	    IMGUI_API void SetFloat(ImGuiID key, float val);
	    IMGUI_API void *
	    GetVoidPtr(ImGuiID key) const; // default_val is NULL
	    ...
	    // - A typical use case where this is convenient for quick hacking
	    // (e.g. add storage during a live Edit&Continue session if you
	    // can't modify existing struct)
	    //      float* pvar = ImGui::GetFloatRef(key);
	    //      ImGui::SliderFloat("var", pvar, 0, 100.0f); some_var +=
	    //      *pvar;
	    IMGUI_API int *GetIntRef(ImGuiID key, int default_val = 0);
	    IMGUI_API bool *GetBoolRef(ImGuiID key, bool default_val = false);
	    IMGUI_API float *GetFloatRef(ImGuiID key, float default_val = 0.0f);
	    IMGUI_API void **GetVoidPtrRef(ImGuiID key,
...
struct ImGuiListClipper {
	    ...
	    // items_count: Use INT_MAX if you don't know how many items you
	    // have (in which case the cursor won't be advanced in the final
	    // step) items_height: Use -1.0f to be calculated automatically on
	    // first step. Otherwise pass in the distance between your items,
	    // typically GetTextLineHeightWithSpacing() or
	    // GetFrameHeightWithSpacing().
	    IMGUI_API void
	    Begin(int items_count,
		  float items_height =
		      -1.0f); // Automatically called by constructor if you
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	    inline ImGuiListClipper(int items_count,
...
struct ImColor {
	    ...
	    ImColor(ImU32 rgba) {
			float sc = 1.0f / 255.0f;
			Value.x =
			    (float)((rgba >> IM_COL32_R_SHIFT) & 0xFF) * sc;
			Value.y =
			    (float)((rgba >> IM_COL32_G_SHIFT) & 0xFF) * sc;
			Value.z =
			    (float)((rgba >> IM_COL32_B_SHIFT) & 0xFF) * sc;
			Value.w =
			    (float)((rgba >> IM_COL32_A_SHIFT) & 0xFF) * sc;
	    }
	    ...
	    inline void SetHSV(float h, float s, float v, float a = 1.0f) {
			ImGui::ColorConvertHSVtoRGB(h, s, v, Value.x, Value.y,
						    Value.z);
			Value.w = a;
	    }
...
struct ImDrawCmd {
	    ...
	    ImTextureID
		TextureId; // 4-8  // User-provided texture ID. Set by user in
	    ...
	    unsigned int
		IdxOffset; // 4    // Start offset in index buffer. Always equal
			   // to sum of ElemCount drawn so far.
	    unsigned int
	    ...
	    void *UserCallbackData; // 4-8  // The draw callback code can access
				    // this.
...
enum ImDrawListFlags_ {
		...
		1 << 2, // Enable anti-aliased edge around filled shapes
...
struct ImDrawList {
		...
		*_OwnerName; // Pointer to owner window's name for debugging
	    ...
	    //   In older versions (until Dear ImGui 1.77) the AddCircle
	    //   functions defaulted to num_segments == 12. In future versions
	    //   we will use textures to provide cheaper and higher-quality
	    //   circles. Use AddNgon() and AddNgonFilled() functions if you
	    //   need to guaranteed a specific number of sides.
	    IMGUI_API void AddLine(const ImVec2 &p1, const ImVec2 &p2,
	    ...
	    IMGUI_API void AddTriangleFilled(const ImVec2 &p1, const ImVec2 &p2,
					     const ImVec2 &p3, ImU32 col);
	    IMGUI_API void AddCircle(const ImVec2 &center, float radius,
				     ImU32 col, int num_segments = 0,
				     float thickness = 1.0f);
	    IMGUI_API void AddCircleFilled(const ImVec2 &center, float radius,
					   ImU32 col, int num_segments = 0);
	    IMGUI_API void AddNgon(const ImVec2 &center, float radius,
		...
		ImU32 col); // Note: Anti-aliased filling requires points to be
			    // in clockwise order.
	    ...
	    // - "uv_min" and "uv_max" represent the normalized texture
	    // coordinates to use for those corners. Using (0,0)->(1,1) texture
	    ...
	    // Stateful path API, add points then finish with PathFillConvex()
	    // or PathStroke()
	    ...
	    inline void PathFillConvex(ImU32 col) {
	      ...
	      // order.
	    inline void PathStroke(ImU32 col, bool closed,
				   float thickness = 1.0f) {
			AddPolyline(_Path.Data, _Path.Size, col, closed,
				    thickness);
			_Path.Size = 0;
	    }
	    IMGUI_API void PathArcTo(const ImVec2 &center, float radius,
				     ...
				     int num_segments = 10);
	    IMGUI_API void
	    PathArcToFast(const ImVec2 &center, float radius, int a_min_of_12,
			  int a_max_of_12); // Use precomputed angles for a 12
					    // steps circle
		...
		void
		    *callback_data); // Your rendering function must check for
				     // 'UserCallback' in ImDrawCmd and call the
				     // function instead of rendering triangles.
	    IMGUI_API void
	    AddDrawCmd(); // This is useful if you need to forcefully create a
	    ...
	    // render out-of-order (e.g. submit FG primitives before BG
	    // primitives)
	    // - Use to minimize draw calls (e.g. if going back-and-forth
	    // between multiple clipping rectangles, prefer to append into
	    // separate channels then merge at the end)
	    // - FIXME-OBSOLETE: This API shouldn't have been in ImDrawList in
	    // the first place!
	    //   Prefer using your own persistent instance of ImDrawListSplitter
	    //   as you can stack them. Using the ImDrawList::ChannelsXXXX you
	    //   cannot stack a split over another.
	    ...
	    // - All primitives needs to be reserved via PrimReserve()
	    // beforehand.
	    IMGUI_API void PrimReserve(int idx_count, int vtx_count);
	    IMGUI_API void PrimUnreserve(int idx_count, int vtx_count);
	    IMGUI_API void PrimRect(const ImVec2 &a, const ImVec2 &b,
				    ImU32 col); // Axis aligned rectangle
	    ...
	    IMGUI_API void PrimQuadUV(const ImVec2 &a, const ImVec2 &b,
				      ...
				      ImU32 col);
	    inline void PrimWriteVtx(const ImVec2 &pos, const ImVec2 &uv,
				     ImU32 col) {
	    ...
	    inline void PrimVtx(const ImVec2 &pos, const ImVec2 &uv,
				ImU32 col) {
...
struct ImDrawData {
	    ...
	    ImDrawList **CmdLists; // Array of ImDrawList* to render. The
				   // ImDrawList are owned by ImGuiContext and
	    ...
	    ImVec2 DisplayPos;	// Upper-left position of the viewport to render
				// (== upper-left of the orthogonal projection
				// matrix to use)
	    ImVec2 DisplaySize; // Size of the viewport to render (==
				// io.DisplaySize for the main viewport)
				// (DisplayPos + DisplaySize == lower-right of
				// the orthogonal projection matrix to use)
	    ImVec2 FramebufferScale; // Amount of pixels for each unit of
				     // DisplaySize. Based on
				     // io.DisplayFramebufferScale. Generally
				     // (1,1) on normal display, (2,2) on OSX
				     // with Retina display.
	    ...
	    IMGUI_API void
	    DeIndexAllBuffers(); // Helper to convert all buffers from indexed
				 // to non-indexed, in case you cannot render
				 // indexed. Note: this is slow and most likely
				 // a waste of resources. Always prefer indexed
				 // rendering!
...
struct ImFontConfig {
	    ...
	    bool FontDataOwnedByAtlas; // true     // TTF/OTF data ownership
				       // taken by the container ImFontAtlas
				       // (will delete memory itself).
	    int FontNo;	      // 0        // Index of font within TTF/OTF file
	    float SizePixels; //          // Size in pixels for rasterizer (more
			      //          or less maps to the resulting font
			      //          height).
	    ...
	    bool PixelSnapH; // false    // Align every glyph to pixel boundary.
			     // Useful e.g. if you are merging a non-pixel
			     // aligned font with the default font. If enabled,
			     // you can set OversampleH/V to 1.
	    ImVec2 GlyphExtraSpacing; // 0, 0     // Extra spacing (in pixels)
				      // between glyphs. Only X axis is
				      // supported for now.
	    ImVec2 GlyphOffset; // 0, 0     // Offset all glyphs from this font
				// input.
	    const ImWchar *
	    ...
	    bool MergeMode; // false    // Merge into previous ImFont, so you
			    // can combine multiple inputs font into one ImFont
			    // (e.g. ASCII font + icons + Japanese glyphs). You
			    // may want to use GlyphOffset.y when merge font of
			    // different heights.
	    unsigned int
		RasterizerFlags; // 0x00     // Settings for custom font
	    ...
	    float RasterizerMultiply; // 1.0f     // Brighten (>1.0f) or darken
				      ...
				      // fonts may be a good workaround to make
				      // them more readable.
	    ImWchar EllipsisChar;     // -1       // Explicitly specify unicode
				  // codepoint of ellipsis character. When fonts
				  // are being merged first specified ellipsis
				  // will be used.
...
struct ImFontAtlasCustomRect {
	    ...
	    unsigned int GlyphID; // Input    // For custom font glyphs only (ID
				  // < 0x110000)
	    ...
	    ImFont
		*Font; // Input    // For custom font glyphs only: target font
...
enum ImFontAtlasFlags_ {
		...
		1 << 2 // Don't build thick line textures into the atlas (save a
		       ...
		       // features uses them, otherwise they will be rendered
...
struct ImFontAtlas {
	    ...
	    IMGUI_API ImFont *
	    AddFontDefault(const ImFontConfig *font_cfg = NULL);
	    IMGUI_API ImFont *
		...
		const ImWchar *glyph_ranges =
		    NULL); // Note: Transfer ownership of 'ttf_data' to
			   ...
			   // the atlas. Set
			   // font_cfg->FontDataOwnedByAtlas=false to keep
	    ...
	    ClearInputData(); // Clear input data (all ImFontConfig structures
			      ...
			      // = all the data used to build the texture and
			      // fonts.
	    IMGUI_API void
	    ClearTexData(); // Clear output texture data (CPU side). Saves RAM
			    // once the texture has been copied to graphics
			    // memory.
	    IMGUI_API void ClearFonts(); // Clear output font data (glyphs
					 // storage, UV coordinates).
	    IMGUI_API void Clear();	 // Clear all input and output.

	    // Build atlas, retrieve pixel data.
	    // User is in charge of copying the pixels into graphics memory
	    // (e.g. create a texture with your engine). Then store your texture
	    // handle with SetTexID(). The pitch is always = Width *
	    // BytesPerPixels (1 or 4) Building in RGBA32 format is provided for
	    // convenience and compatibility, but note that unless you manually
	    // manipulate or copy color data into the texture (e.g. when using
	    // the AddCustomRect*** api), then the RGB pixels emitted will
	    // always be white (~75% of memory/bandwidth waste.
	    IMGUI_API bool
	    Build(); // Build pixels data. This is called automatically for you
	    ...
	    bool IsBuilt() const {
			return Fonts.Size > 0 && (TexPixelsAlpha8 != NULL ||
	    ...
	    // Helpers to retrieve list of common Unicode ranges (2 value per
	    // range, values are inclusive, zero-terminated list) NB: Make sure
	    // that your string are UTF-8 and NOT in your local code page. In
	    // C++11, you can create UTF-8 string literal using the u8"Hello
	    // world" syntax. See FAQ for details. NB: Consider using
	    ...
	    // for your own purposes. After calling Build(), you can query the
	    // rectangle position and render your pixels. You can also request
	    // your rectangles to be mapped as font glyph (given a font +
	    // Unicode point), so you can render e.g. custom colorful icons and
	    // use them as regular glyphs. Read docs/FONTS.md for more details
	    // about using colorful icons. Note: this API may be redesigned
	    // later in order to support multi-monitor varying DPI settings.
	    IMGUI_API int AddCustomRectRegular(int width, int height);
	    IMGUI_API int
	    AddCustomRectFontGlyph(ImFont *font, ImWchar id, int width,
	    ...
	    int TexDesiredWidth; // Texture width desired by user before
				 // Build(). Must be a power-of-two. If have
				 // many glyphs your graphics API have texture
				 // size restrictions you may want to increase
				 // texture width to decrease height.
	    int TexGlyphPadding; // Padding between glyphs within texture in
				 // pixels. Defaults to 1. If your rendering
				 // method doesn't rely on bilinear filtering
				 // you may set this to 0.

	    // [Internal]
	    // NB: Access texture data via GetTexData*() calls! Which will setup
	    // a default font for you.
	    unsigned char *TexPixelsAlpha8; // 1 component per pixel, each
					    // component is unsigned 8-bit.
					    // Total size = TexWidth * TexHeight
	    unsigned int *TexPixelsRGBA32;  // 4 component per pixel, each
					   // component is unsigned 8-bit. Total
		...
		CustomRects; // Rectangles for packing custom texture data into
			     // the atlas.
	    ...
	    int PackIdMouseCursors; // Custom texture rectangle ID for white
				    // pixel and mouse cursors
	    int PackIdLines;	    // Custom texture rectangle ID for baked
				    // anti-aliased lines
...
struct ImFont {
		...
		IndexAdvanceX; // 12-16 // out //            // Sparse.
			       ...
			       // (cache-friendly for CalcTextSize functions
			       // which only this this info, and are often
	    ...
	    ImVector<ImWchar>
		IndexLookup; // 12-16 // out //            // Sparse. Index
		...
		*FallbackGlyph; // 4-8   // out // = FindGlyph(FontFallbackChar)
	    ...
	    ImFontAtlas *ContainerAtlas; // 4-8   // out //            // What
					 // we has been loaded into
	    const ImFontConfig
		*ConfigData; // 4-8   // in  //            // Pointer within
			     // ContainerAtlas->ConfigData
	    short ConfigDataCount; // 2     // in  // ~ 1        // Number of
				   // ImFontConfig involved in creating this
				   // font. Bigger than 1 when merging multiple
				   // font sources into one ImFont.
	    ImWchar FallbackChar;  // 2     // in  // = '?'      // Replacement
				   // character if a glyph isn't found. Only set
				   // via SetFallbackChar()
	    ImWchar EllipsisChar;  // 2     // out // = -1       // Character
				   // used for ellipsis rendering.
	    bool DirtyLookupTables; // 1     // out //
	    float Scale; // 4     // in  // = 1.f      // Base font scale,
			 // multiplied by the per-window font scale which you
			 // can adjust with SetWindowFontScale()
	    float Ascent,
		Descent; // 4+4   // out //            // Ascent: distance from
			 // top to bottom of e.g. 'A' [0..FontSize]
	    int MetricsTotalSurface; // 4     // out //            // Total
				     // surface in pixels to get an idea of the
				     // font rasterization/texture cost (not
	    ...
	    float GetCharAdvance(ImWchar c) const {
			return ((int)c < IndexAdvanceX.Size)
				 ? IndexAdvanceX[(int)c]
				 : FallbackAdvanceX;
	    }
	    ...
	    // 'max_width' stops rendering after a certain width (could be
	    // turned into a 2d size). FLT_MAX to disable. 'wrap_width' enable
	    // automatic word-wrapping across multiple lines to fit into given
	    ...
	    IMGUI_API void RenderChar(ImDrawList *draw_list, float size,
				      ImVec2 pos, ImU32 col, ImWchar c) const;
	    IMGUI_API void
	    RenderText(ImDrawList *draw_list, float size, ImVec2 pos, ImU32 col,
	    ...
	    AddRemapChar(ImWchar dst, ImWchar src,
			 bool overwrite_dst =
			     true); // Makes 'dst' character/glyph points to
